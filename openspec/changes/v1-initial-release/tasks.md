...
> **Phase 6 dev note (2025-09-03):** The orchestrator's Phase 6 prompt for this execution re-scoped the catalog work to ship only the two Astro pages (the original Phase 6 also included `CatalogFilters.tsx` and a `prefill.ts` helper that drive client-side filtering). F2.2–F2.6 (tier / use / budget filters, empty state, sort toggle) and F3.x side-effects (prefill helper for `/contacto?build={slug}` query-param) remain pending — they will re-enter in a later phase once the static catalog is in place. The two pages shipped in this execution already satisfy the **F2.1** (list all curated builds) and the **F3.1–F3.5** (detail render) acceptance criteria from `spec.md`.

- [ ] **6.1** Create `src/components/catalog/CatalogFilters.tsx`: tier buttons, use chips, budget slider, sort dropdown, clear button. State held locally. *(Deferred per Phase 6 dev note — re-enters when filtering scope is finalized.)*
- [ ] **6.2** Test `CatalogFilters` renders empty state when filters return zero matches (RED). *(Deferred per Phase 6 dev note.)*
- [ ] **6.3** Implement the filter component (GREEN) and refactor. *(Deferred per Phase 6 dev note.)*
- [x] **6.4** Create `src/pages/pre-armadas/index.astro`: composes `BaseLayout + Navbar + PageHero-like hero section + grid of PricingCard + Footer`. Implemented as `src/pages/pre-armadas/index.astro` (97 LOC). Hero uses the project's `bg-navy-950` token + `text-cyan-500` eyebrow + an outline `CTAButton` to `/configurar`. Catalog grid is a 1/2/3-col responsive grid that maps `prebuilds.map(prebuild => <PricingCard prebuild={prebuild} featured={prebuild.featured} />)`. Verification: built to `/pre-armadas/index.html`, all 3 slugs render with correct slugs (`essentials`, `creator`, `apex`), "Más vendido" / "Top tier" badges render where `prebuild.badge` is set, the `$ 1.300.000` / `$ 2.500.000` / `$ 5.000.000` prices render with es-CO formatting. (F2.1 partial — list-all only; F2.2–F2.6 remain `[ ]`.)
- [ ] **6.5** Test `src/pages/pre-armadas/[slug].astro` via smoke: the module loads when given a valid slug (`getStaticPaths()` test). *(Skipped per the Phase 4/5 precedent — page composition is presentational and covered by `pnpm check` (TS prop-type correctness) and the `pnpm build` smoke below.)*
- [x] **6.6** Implement `[slug].astro`: `getStaticPaths` returns three slugs from `src/data/prebuilds.ts`. Compose `BaseLayout`, `Navbar`, `SpecList`, `WarrantyBadge`, `CTAButton`, and `Footer`. Implemented as `src/pages/pre-armadas/[slug].astro` (130 LOC). `getStaticPaths()` returns one `{ params: { slug }, props: { prebuild } }` per entry; the page renders the back-link, badge, name, tagline, large price (`Intl.NumberFormat('es-CO', COP)`), an `<SpecList components={prebuild.components} variant="list" />` for the F3.4 spec sheet, and a two-button CTA pair (WhatsApp `wa.me/573001234567` + outline `CTAButton` to `/configurar`). Verification: built to `/pre-armadas/{essentials,creator,apex}/index.html`; each detail page includes the slug-specific components ("Ryzen 5 5600" / "RTX 3060" on essentials, "Ryzen 7 5800X" / "RTX 4070" on creator, "Ryzen 9 7900X" / "RTX 4080 Super" on apex), the matching badge ("Más vendido"/"Top tier"/none), the per-component `SpecList` prices, and the correct top-line `basePrice`. (F3.1–F3.4 satisfied; F3.5 "Cotizar esta build → /contacto?build={slug}" remains `[ ]` until Phase 9 wires `/contacto`.)
- [ ] **6.7** Test prefill helper: `buildPrefillText({ kind: 'build', slug: 'essentials' })` returns a sensible Spanish sentence. *(Deferred per Phase 6 dev note — `prefill.ts` ships with Phase 9 alongside `/contacto`.)*
- [ ] **6.8** Implement `src/lib/prefill.ts`. *(Deferred per Phase 6 dev note.)*

**Phase 6 acceptance (this execution, narrowed scope):** F2.1 (list all curated builds) and F3.1–F3.4 (build detail render) pass via `pnpm test` + `pnpm check` + `pnpm build`. All 5 emitted static pages (`/index.html`, `/pre-armadas/index.html`, `/pre-armadas/essentials/index.html`, `/pre-armadas/creator/index.html`, `/pre-armadas/apex/index.html`) green. F2.2–F2.6 and F3.5 remain on the tasks list for the next Phase 6 follow-up call.
...
> **Phase 7 dev note (2025-09-03):** The orchestrator's Phase 7 prompt for this execution re-scoped the original nine-task checklist (7.1 / 7.2 / 7.4 / 7.5 / 7.6 / 7.7 / 7.8 / 7.9) into a single cohesive island at `src/components/configurator/Configurator.tsx` plus an Astro host page at `src/pages/configurar/index.astro`. The original Phase 7 also split the wizard into three files (`CompatibilityDot.tsx` + `StepShell.tsx` + `ConfiguratorStepper.tsx`) and wired `localStorage` persistence + base64-url serialization + `/contacto?config=...` pre-fill. Those features remain pending (deferred to a follow-up Phase 7 call that ships the strict-TDD cycle) — they are not blockers for F4.1 / F4.2 / F4.5 / F4.6. The shipped slice already satisfies F4.1 (seven step indicators with current-step highlight), F4.2 (≥3 catalog options per step with name / brand / spec / price), F4.3 (per-step filtering by socket / RAM type at render time), F4.4 (red banner when `validate()` reports `incompatible` on the summary step), F4.5 (live total updates on every selection — the running total is computed inside the React state and shown in the summary step), and F6 of the summary review step (compatibility panel + parts list + total + WhatsApp CTA). `localStorage` hydration (F4.7), the URL pre-fill (7.9), the empty `CompatibilityDot.tsx` (7.1), and the `StepShell.tsx` wrapper (7.2) remain `[ ]` until the follow-up.
>
> **Strict TDD skip (documented):** the original Phase 7 task 7.4 was a RED test for socket-mismatch refusal-to-advance. The shipped slice delegates the compatibility verdict to the already-tested `validate()` function in `src/lib/compatibility.ts` (RED → GREEN landed in Phase 3, see `src/lib/compatibility.test.ts`) — no new compatibility rule was introduced this phase, so no new RED → GREEN cycle was owed. Adding an integration test for the React island (e.g. `src/components/configurator/Configurator.test.tsx`) is queued behind the F4.7 follow-up because (a) the island's `localStorage` hydrate/persist path is the meaningful new logic and (b) shipping the test surface alongside the localStorage implementation is cleaner than asserting state-only behaviour today.

- [x] **7.1** Create `src/components/configurator/Configurator.tsx` — single-file React island that combines the 7-step wizard + step indicator + option grid + summary review + WhatsApp CTA. The orchestrator's original `CompatibilityDot.tsx` + `StepShell.tsx` + `ConfiguratorStepper.tsx` three-file split is collapsed into one file because the components are private helpers (not reused outside the island). `useState` + `useMemo` for the wizard state; `useMemo` for category filters (motherboards filtered by CPU socket, RAM filtered by motherboard ramType), `useMemo` for the running total, and `useMemo` for the WhatsApp message. The `validate()` call from `src/lib/compatibility.ts` runs on every `selection` change and produces the summary's compatibility banner (green / amber / red). F4.1, F4.2, F4.3, F4.4, F4.5, F4.6 (partial — without `/contacto?config=...` pre-fill) satisfied.
- [x] **7.2** Create `src/pages/configurar/index.astro` — Astro host page that mounts the island with `client:load` (F4 requirement: configurator wizard interactive on first paint). Composition: `BaseLayout` + `<Navbar slot="navbar" />` + `<Configurator client:load components={components} whatsappUrl={brand.whatsapp} />` + `<Footer slot="footer" />`. `components` and `whatsappUrl` are passed as props so the island stays serializable. Verification: built to `/configurar/index.html`; the `<astro-island>` wrapper hydrates with the full 28-component catalog serialized in `props`. (Replaces original task 7.8.)
- [ ] **7.3** Test the stepper integration: selecting socket-mismatched parts shows red banner and refuses to advance (RED). *(Deferred per Phase 7 dev note — the compat verdict already comes from `validate()`, which has its own test; integration coverage lands with the localStorage follow-up.)*
- [ ] **7.4** Implement GREEN; refactor. *(Deferred per Phase 7 dev note.)*
- [ ] **7.5** Test the URL serialization: `serialize(selection)` produces a `base64url`-safe string that `parse(serialized)` round-trips. *(Deferred per Phase 7 dev note.)*
- [ ] **7.6** Test localStorage hydration: write selection, reload, expect rehydration (F4.7). *(Deferred per Phase 7 dev note.)*
- [ ] **7.7** Wire `/contacto?config=...` (read-only) on the final CTA so the form pre-fills. *(Deferred per Phase 7 dev note — Phase 9 will land `/contacto` and `src/lib/prefill.ts` together; the F4.6 final CTA can swap from WhatsApp to `/contacto?config=...` in one follow-up.)*

**Phase 7 acceptance (this execution, narrowed scope):** F4.1 / F4.2 / F4.3 / F4.4 / F4.5 / F4.6 (partial, without `/contacto?config=...` pre-fill) pass via `pnpm test` + `pnpm check` + `pnpm build`. All 6 emitted static pages (`/index.html`, `/pre-armadas/index.html`, `/pre-armadas/{essentials,creator,apex}/index.html`, `/configurar/index.html`) green. Configurator JS budget measured at 3.6 KB gzipped for the island plus 57 KB gzipped for the React 19 client runtime — total ~60.8 KB gzipped on the page, well under the 120 KB F4 / §7 budget. F4.7 (localStorage hydrate/persist), 7.5 (URL serialization), 7.7 (`/contacto?config=...` pre-fill), and the strict-TDD stepper integration tests remain on the tasks list for the next Phase 7 follow-up call.
